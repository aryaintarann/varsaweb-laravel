        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                @forelse($services as $index => $service)
                    ...
                    <div class="card-hover group bg-gradient-to-br {{ $color['gradient'] }} dark:from-slate-800 dark:to-slate-700/50 rounded-3xl border border-white/80 dark:border-slate-700/80 overflow-hidden">
                        <div class="p-8 text-center">
                            <div class="w-16 h-16 rounded-2xl {{ $color['bg'] }} dark:bg-white/10 flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 shadow-sm mx-auto">
                            ...
                            <p class="text-slate-600 dark:text-slate-300 leading-relaxed mb-4">{{ $service->short_description }}</p>
                        </div>
                        <div class="px-8 py-4 bg-white/40 dark:bg-slate-800/40 backdrop-blur-sm border-t border-white/50 dark:border-slate-700/50 text-center">
                            <a href="{{ route('contact') }}?service={{ urlencode($service->title) }}" class="text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:text-indigo-800 dark:hover:text-indigo-300 inline-flex items-center gap-2 transition-colors duration-200 group/link">
                ...
                @empty
                    <div class="col-span-full text-center py-20 bg-white dark:bg-slate-800 rounded-3xl border border-slate-100 dark:border-slate-700">
                        ...
                        <h3 class="font-semibold text-slate-900 dark:text-white">No Services Available Yet</h3>
                        <p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Check back later for updates to our offerings.</p>
                    </div>
                @endforelse
            </div>
